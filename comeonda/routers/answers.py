...
from models import User, Question, Option, Answer, PointHistory, Notification
...
async def submit_answer(
    ...
    try:
        ...
        result = await db.execute(select(Option).where(Option.id == answer.option_id))
        ...
        result = await db.execute(
            select(Answer).where(
                Answer.user_id == user.id, Answer.question_id == answer.question_id
            )
        )
        ...
        user.points -= 100
        new_answer = Answer(
            user_id=user.id, question_id=answer.question_id, option_id=answer.option_id
        )
        db.add(new_answer)
        point_history = PointHistory(
            ...
            description="Question entry fee",
        )
        ...
        logging.error("Answer submission failed", exc_info=True)
...
async def my_history(
    ...
    try:
        ...
        logging.error("Unable to fetch answer history", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
